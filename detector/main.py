...
def main():
    ...
    if args.test == 1:
        ...
        dataset = data.DataBowl3Detector(
            datadir,
#             'full.npy',
            config_training['preprocess_path'],
            config,
            phase='test',
...
def train(data_loader, net, loss, epoch, optimizer, get_lr, save_freq, save_dir):
    ...
    # for data, target, coord in data_loader:
        # print(data.shape)
        # print(target.shape)
        # print(coord.shape)
        ...
        loss_output = loss(output, target) # 0.5.0版本之后loss的写法有改变，作者是基于0.1.10，会出现上述错误
        optimizer.zero_grad()
