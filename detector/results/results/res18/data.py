...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase = 'train',split_comber=None):
        ...
        for idx in idcs:
            # print(np.load(os.path.join(data_dir, '%s_label.npy' %idx))) # 全是[[0 0 0 0]]，说明是luna_prepare的原因
            l = np.load(os.path.join(data_dir, '%s_label.npy' %idx), allow_pickle=True)
            if np.all(l==0):
                l=np.array([])
            labels.append(l)
            # print(labels) # 目前np.all(l==0)，说明%s_label.npy的数据没有导进来，或者%s_label.npy本身就生成有问题，正排查...
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        xx,yy,zz = np.meshgrid(np.linspace(normstart[0],normstart[0]+normsize[0],int(np.ceil(self.crop_size[0]/self.stride))),
                           np.linspace(normstart[1],normstart[1]+normsize[1],int(np.ceil(self.crop_size[1]/self.stride))),
                           np.linspace(normstart[2],normstart[2]+normsize[2],int(np.ceil(self.crop_size[2]/self.stride))),indexing ='ij')
        coord = np.concatenate([xx[np.newaxis,...], yy[np.newaxis,...],zz[np.newaxis,:]],0).astype('float32')
